	...
	automationNotifier(5)
{
	...
	rangeRemapMode->addOption("Absolute", (int) RangeRemapMode::Absolute)->addOption("Proportional", (int) RangeRemapMode::Proportional);
...
void Automation::updateRange()
{
	if (valueRange->enabled)
	{
		...
		if (rangeRemapMode != nullptr)
		{
			...
			for (auto& k : items) k->setValueRange(valueRange->x, valueRange->y, rrm == RangeRemapMode::Proportional);
		}
	}
	else
	{
